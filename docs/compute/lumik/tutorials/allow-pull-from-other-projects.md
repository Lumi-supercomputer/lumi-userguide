!!! success "Basic level"
    You need a knowledge of OpenShift CLI tool [oc](../usage/cli.md) or the LUMI-K [web interface](../usage/getting_started.md)

# Allow pulls of images from one LUMI-K project to another

Every LUMI-K project has a private image registry space. Any image compatible with the [OCI](https://opencontainers.org/) format can be stored in the registry. Common uses for the registry are to store [images built by LUMI-K](../images/creating.md#using-LUMI-K-to-build-container-images) or cache upstream images.
...
The objective of this tutorial is to explain how to achieve an extra use case, pulling images from another project within LUMI-K. The use case for this is when several projects are used to deploy the same infrastructure thus sharing the same images. In the example below, the first project will be production (`bajor-prod`), the other one will be for testing (`bajor-test`). Both need to share the same registry to make the test and deployment easier and simpler to follow.
...
1. In order to test it, one just need to deploy a Pod in `bajor-test` that uses an image from `bajor-prod` (similar to `image-registry.apps.2.LUMI-K.csc.fi/bajor-prod/image-name`).
