!!! success "Basic level"
    You need to familiarize yourself with the LUMI-K [web interface](../usage/getting_started.md)

    You need knowledge on Git and GitHub. Also, a GitHub account is required

# Webhooks

Webhooks are URLs that allow triggering actions in a system. LUMI-K supports webhooks to trigger rebuilds. This means that each BuildConfig is listening to a particular URL that includes a secret (more about that later), and that when this URL is called, a build will be triggered for a default branch.
...
!!! Warning "Default branch names do not match"
    ...
    You need to make sure that the branch names match between LUMI-K and GitHub. In LUMI-K, the default branch name is `master`, but in GitHub, the default branch name is `main`. This means that&mdash;by default&mdash;any change in a GitHub branch named `main` will be ignored by LUMI-K.

    If you want changes in `main` to be picked up by LUMI-K, you need to:
