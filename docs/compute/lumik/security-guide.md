...
LUMI-K applications are exposed to the Internet, and
their security should be treated with an appropriate care.
The user on whose account a service is running in LUMI-K is
responsible for its security. See the [terms of use](https://LUMI-K.csc.fi/terms_of_use.html) for details about the
responsibilities.

This guide should be treated as the baseline which must be taken
in account rather than a checklist for perfect security.

Measures that tighten the security of the services running in LUMI-K can be
...
- **[Default Pod resource limits](../LUMI-K/usage/projects_and_quota.md#default-pod-resource-limits)**
...
the subdomain `*.2.Lumi-Kapp.fi` (e.g. `coolservice.2.Lumi-Kapp.fi`), the default
wildcard TLS certificate provided by LUMI-K can be used directly. Otherwise,
...
The IP for all outgoing customer traffic is `86.50.229.150`. By opening a firewall to this IP, you will let in all traffic coming from any LUMI-K project. It is advised to not solely rely in IP filtering for security, but use this a secondary measure, like for example an OAuth authentication system.
