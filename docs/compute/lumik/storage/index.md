...
This article introduces the storage options available in CSC's LUMI-K environment. Using the storage requires an account, a CSC project, a LUMI-K Project, and at least a Pod. In LUMI-K a project is a Kubernetes namespace with additional annotations.
...
A snapshot represents the state of the storage volume within a cluster at a specific point in time. Volume snapshots can be used to provision a new volume and help protect against data loss in OKD. LUMI-K support Container Storage Interface (CSI) volume snapshots by default and the default volume snapshot class name is standard-csi.
