...
* If the domain name is not included, docker assumes it is default public docker registry, `docker.io`. But there are other registries, for example  `image-registry.apps.2.LUMI-K.csc.fi` is LUMI-K's private docker registry.
...
[
    {
        ...
        "RepoTags": [
            "nginx:latest",
            "image-registry.apps.2.LUMI-K.csc.fi/tristan-tests/nginx-is:latest"
        ],
        "RepoDigests": [
...
Check out our [LUMI-K FAQ](../../../support/faq/index.md#LUMI-K) and [LUMI-K tutorials](../tutorials/index.md) for more information