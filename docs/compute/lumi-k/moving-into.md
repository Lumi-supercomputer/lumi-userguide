...
strengths of each platform. Both Pouta and Kubernetes-based platforms, such as LUMI-K, are Cloud
...
infrastructure as Virtual Machine. LUMI-K, on the other hand is classifies as a Platform-as-a-Service (PaaS), where users can deploy
...
Using a container orchestration service like LUMI-K that is built on top of Kubernetes, makes the
deployment of applications easy. In LUMI-K you can manage your applications directly using a web
interface accessible through a web browser. LUMI-K provides a platform for you to host your own applications
and make them accessible over the web. LUMI-K can run many different kinds of applications and workloads, from web servers
and databases to data analysis pipelines.

Some example use cases that are well suited for LUMI-K include:

* Hosting an interactive web application or a regular website. LUMI-K comes with many of the most common
...
LUMI-K and Openshift containers are not allowed to run with root access, which may provide some limitations
...
[What is LUMI-K?](LUMI-K-what-is.md)

[Container clouds fundamentals](https://LUMI-K-course.a3s.fi/index.html#1)
