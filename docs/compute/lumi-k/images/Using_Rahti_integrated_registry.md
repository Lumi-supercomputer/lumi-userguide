...
It is possible to manually cache images in LUMI-K. This could be useful to remove
an external dependency or improve performance.

The process is simple:

1. [Install](../usage/cli.md#the-command-line-tools-page-in-the-LUMI-K-web-ui) and [login with OC](../usage/cli.md#the-command-line-tools-page-in-the-LUMI-K-web-ui).

1. With a terminal, connect to the LUMI-K registry:
    ```sh
    sudo docker login -p $(oc whoami -t ) -u unused image-registry.apps.2.LUMI-K.csc.fi
    ```

    _Alternatively, you can access to this address: <https://oauth-openshift.apps.2.LUMI-K.csc.fi/oauth/token/display> to request
    a token. Once connected, display and copy the token. The command will be:_

    ```sh
    sudo docker login -p <YOUR_TOKEN> -u unused image-registry.apps.2.LUMI-K.csc.fi
    ```
...
2. Tag the image you want to push:
   ```sh
   sudo docker tag almalinux:10 image-registry.apps.2.LUMI-K.csc.fi/{YOUR_Lumi-K_PROJECT_NAME}/almalinux:<tag>
   ```
   _Replace {YOUR_Lumi-K_PROJECT_NAME} by the name of your project._
   _Please note that YOUR_Lumi-K_PROJECT_NAME here is the LUMI-K project name (AKA namespace name), and does not refer to CSC project._

4. Push your image:
   ```sh
   sudo docker push image-registry.apps.2.LUMI-K.csc.fi/{YOUR_Lumi-K_PROJECT_NAME}/almalinux:<tag>
   ```

You should be able to see your images in your project:
![Image Streams](../../img/image_streams_Lumi-K4.png)
...
!!! warning "Troubleshooting"
    ...
    ```
    unknown: unexpected status from HEAD request to https://image-registry.apps.2.LUMI-K.csc.fi/v2/{YOUR_Lumi-K_PROJECT_NAME}/{YOUR_IMAGE_NAME}/manifests/sha256:834e7b0d913dd73e8616810c2c3a199cd8a3618e981f75eea235e0920d601ce4: 500
    ```
...
LUMI-K allows fine-grained control over access to the integrated image registry, enabling management of access based on [user authentication](https://docs.redhat.com/en/documentation/openshift_container_platform/4.17/html/authentication_and_authorization/index).
