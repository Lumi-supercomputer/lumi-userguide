...
will be deleted if you Pod is deleted or migrated to another node. It is declared directly in the Pod definition:
...
It is possible to make an `emptyDir` even faster by using live memory as storage medium instead (i.e., **tmpfs**)
...
much memory — the contents of the tmpfs volume contribute to the limit. You can memory backed emptyDir create one by
adding `medium: Memory` under `emptyDir`. It is recommended to configure the `sizeLimit` to a value lower than the 
Pod memory limit.
