...
LUMI-K can be used via the command line either with OpenShift's _oc_ tool
...
The oc tool is a single binary that only needs to be included in your _path_. Instructions for downloading the oc tool including the download link for several platforms and operating systems can be found in the [Command Line Tools](https://console.LUMI-K.csc.fi/command-line-tools) page in the web interface:
...
```bash
oc login https://api.2.LUMI-K.csc.fi:6443 --token=<secret access token>
```
