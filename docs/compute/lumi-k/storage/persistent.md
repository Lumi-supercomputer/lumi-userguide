--8<-- "Lumi-K_rwx_storage.md"

# Persistent volumes

**Persistent volumes** are storage which persist during & after pod's lifetime.

Persistent volumes in LUMI-K are stored in a resilient storage such as CEPH. They are created by using a **PersistentVolumeClaim**. When a
new claim is made, and **a Pod mounts it**, a new volume space will be created.

!!! info "A change from LUMI-K 1 version"

    In production LUMI-K a new persistent volume will remain in **"⏳ Pending"** until any Pod mounts it. This is a change from LUMI-K 1 where the volume was created right away.

There is one storage class available in LUMI-K:
