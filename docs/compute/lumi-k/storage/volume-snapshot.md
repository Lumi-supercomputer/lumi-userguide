...
LUMI-K offers two methods for provisioning snapshots: through web interface and by using the CLI.

### Prerequisites

- An active project in LUMI-K
...
2. Create a PVC
3. Mount the PVC to deployment (in LUMI-K PVC is only created after that is mount to a deployment)
