...
bucket name is created. The `rclone.conf` should be set on your local system like described above example.
For example, `rclone.conf` path could be located in `~/.config/rclone/rclone.conf`. More information on creating 
[LUMI-O bucket](../../../../storage/lumio/clients-general.md). This script will backup an application deployed in LUMI-K. 
