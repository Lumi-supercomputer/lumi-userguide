When using LUMI-K, you might need to build your own container images before deploying them. Possible reasons for doing 
...
want run it in LUMI-K as containers.
...
The methods below use LUMI-K to build container images.
...
S2I lets LUMI-K handle the build process using its default builder images for different runtime environment (e.g., Python, Node.js, Java, Go, 
PHP, Ruby). You can see the list of available build images in LUMI-K by running the following command:
...
![Import git project](../../img/lumik_import_git.png)
...
![Import git error](../../img/lumik_private_repo_error.png)
...
* Next, you need to provide authentication credentials. Two options are possible, a token or an ssh private key:
  1. **Option 1: Using a Token for Git Authentication**
     ...
     2. **Add the Token to LUMI-K:**
           ...
           ![Create git token](../../img/lumik_create_git_token.png)
  ...
  2. **Option 2: Using a Private SSH Key**
     ...
     3. **Add the Private SSH Key to Rahti:**
        ...
        ![Create git token](../../img/lumik_create_git_key.png)
...
1. In LUMI-K console, navigate to `Builds > BuildConfigs` and click on your BuildConfig.
