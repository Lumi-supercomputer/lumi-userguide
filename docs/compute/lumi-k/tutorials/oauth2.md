...
# How to deploy OAuth2 Proxy in LUMI-K
...
First, we will deploy a very simple web application, this [flask demo](https://github.com/CSCfi/LUMI-K-flask-hello). Follow the instructions
...
You can use our [NGINX image](https://github.com/CSCfi/nginx-okd) to run NGINX on LUMI-K (OpenShift 4)

With LUMI-K, you can directly build the image. More information [here](../images/creating.md#using-the-source-to-image-mechanism)
...
```sh
oc create route edge flask-demo --service=nginx-okd --hostname=demo-oauth2.2.Lumi-Kapp.fi --insecure-policy='Redirect' --port=8081
```

!!! info
    You can set your own `--hostname`. In this example, we are using LUMI-K default domain `2.Lumi-Kapp.fi`.
...
data:
  default.conf: |
    server {
      ...
      server_name demo-oauth2.2.Lumi-Kapp.fi; # The same as the --hostname created previously in the Route
...
- **Homepage URL**: The homepage of your web application. In this example, it will be the route of the NGINX reverse proxy (`https://demo-oauth2.2.Lumi-Kapp.fi`)
- **Authorization callback URL**: Your applications's callback URL. In this example, it will be like `https://demo-oauth2.2.Lumi-Kapp.fi/oauth2/callback`
  ...
  template:
    ...
    spec:
      ...
      - args:
        ...
        - --redirect-url=https://demo-oauth2.2.Lumi-Kapp.fi/oauth2/callback
        - --email-domain=*
        - --reverse-proxy=true
        - --upstream=static://200
...
so https://demo-oauth2.2.Lumi-Kapp.fi for this example).
