...
and deploy it to LUMI-K. The following steps will quickly get you started:

Clone the source repository for the sample application.

```
git clone https://github.com/CSCfi/LUMI-K-bc-example.git
cd LUMI-K-bc-example/
```

Login to LUMI-K if you haven't done so. You can copy your login command from the LUMI-K UI.

```
 oc login https://api.2.LUMI-K.csc.fi:6443 --token=<your_token>
```
...
We have all the necessary object definitions in the `LUMI-K-bc-example` project under the `k8s-api-objs.yaml`.
...
by following the route which should look like `http://dockerfile-example-<your_project_name>.2.Lumi-Kapp.fi/`.
