...
1. A route that will expose the service to the internet by redirecting
   traffic from `<myservice>.2.Lumi-Kapp.fi` to the service object.
...
Using project "my-project-with-unique-name" on server "https://api.2.LUMI-K.csc.fi:6443".
```
...
The name of the project needs to be unique across the LUMI-K container cloud, and
...
If you are a member of multiple CSC projects with access to LUMI-K, the description of the
...
You can now go to your browser and type the address you set: `<myservice>.2.Lumi-Kapp.fi`. It should return you the Apache test page:
...
+ `.2.Lumi-Kapp.fi` unless otherwise specified in `spec.host`.
