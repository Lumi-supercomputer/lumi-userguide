...
In order to test this in LUMI-K, one only needs to login in LUMI-K, select the correct project, and run:
...
**NOTE**: The code must be in a git repository and LUMI-K must be able to clone it.

The end result will be an image called `multi-stage-build` stored in the internal LUMI-K registry of the project you selected. This image can then be used in a LUMI-K deployment using the image stream option when deploying an image.
