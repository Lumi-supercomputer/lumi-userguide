...
What other users can see about your account information:   
  ...
  - Your project members can see your first name, last name, and username with the project information 
  ...
  - All users can see the last time you logged in to the system with the `last` command.
...
- Contents of home directory is only visible for the user themselves. 
...
The commands you run on shared nodes are visible for the other users on the same node. If the command launches a process that stays running, other users are able to see the details of the running process, e.g. your username, the command that launched the process including command line arguments, and details about resource usage of that command. 
...
Shared nodes are all the LUMI login nodes, and the nodes on the shared computing partitions (including the login node shell in LUMI web interface). 
...
- Pay attention to the command line arguments that you use: As the command line arguments can in many cases be viewed by other users in the system, don't give command line arguments that couldn't be seen by other users.  
