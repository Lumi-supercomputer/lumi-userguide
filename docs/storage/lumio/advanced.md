...
Consult the manual pages for the tools for additional details.
...
Lifecycle configuration and write once, read many functionality, which are not really part of "just" s3 storage. 

It worth keeping the above in mind, as many people use S3 and Amazon S3 interchangeably
...
files / object can be retrived from the directory / bucket, but it's not possible to list the content.
...
Note that the authorization headder of any request is checked before any access rules are 
