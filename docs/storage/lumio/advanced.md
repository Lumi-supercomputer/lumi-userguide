...
Sharing data, e.g. datasets or research results, is (in principle) easy in the object storage. One should pay attention to understand what they are doing, though, especially before changing access rights to the data (with ACLs or Policies), or to not unintendedly publish something that shouldn't be publicly accessible.
...
The data in LUMI-O can be shared in a variety of ways:
...
With _Policies_ it's also possible to e.g. restrict object access to specific IP:s. 
...
make objects and buckets public or private. 
