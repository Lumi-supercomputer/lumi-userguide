...
systems. Each of these provides a storage capacity of 20 PB with a bandwidth of
240 GB/s. Each Lustre file systems is composed of 1 MDS (metadata server) and
32 Object Storage Targets (OSTs).

However, if you are limited by the quota on the number of files, 
...
TB-hours. In practice, 1TB that stays for 1 hour on the file system will consume
