...
| **LUMI Terms of Use** | The [LUMI Terms of Use](https://lumi-supercomputer.eu/termsandpolicies/) prohibit credential sharing, which means that you should never share your LUMI credentials or let any other party access LUMI using them.<br><br>Allowing a coding agent to run commands without prompting for approval is considered credential sharing, since this allows the model provider to execute arbitrary code on LUMI. | Do not let AI agents execute commands on LUMI without always prompting for approval. Select your tools carefully, and ensure that you understand how they work. If you are unsure whether a tool breaks the LUMI Terms of Use, do not use it on LUMI. |
