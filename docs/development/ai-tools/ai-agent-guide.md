...
| **Agent not understanding system limits**| The agent might not be aware of certain limits on LUMI like [storage quotas](./../../storage/index.md#lumi-network-file-system-disk-storage-areas) or [slurm partitions](./../../runjobs/scheduled-jobs/partitions.md). | Validate the error messages before sending tickets to user support. Consider using the [LUMI AI Factory MCP server](./../../laif/software/agent-infrastructure.md) to make the agent aware of LUMI documentation. |
