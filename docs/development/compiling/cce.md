...
compilers. The Cray Fortran compiler supports the Fortran 2018 standard, with
newer versions also supporting most of Fortran 2023 (and Fortran 2023 support 
almost complete in CCE 20.0.0).
The C/C++ compiler is C17 and C++17 compliant. They are based on regular clang, so 
support the same language standards as the corresponding regular clang/LLVM compiler. 
An overview of language support in the regular clang/LLVM compiler is available 
[on this page for C](https://clang.llvm.org/c_status.html) and
[on this page for C++](https://clang.llvm.org/cxx_status.html).
Invoking these compilers is done
through the `ftn`, `cc` and `CC` [compilers wrappers][wrappers].

CCE has support for the full OpenMP 5.0 specification as well as partial
support for OpenMP 5.1, 5.2 and 6.0. 
PGAS languages (UPC and Fortran coarrays) are also
integrated.

## Overview

Currently, the CCE compilers use a HPE Cray-developed Fortran frontend and the regular
clang C/C++ frontend. As a result of this, compiler options for C/C++ and Fortran 
do not completely align as the following table shows:
...
:material-help-circle-outline: `man intro_openmp` or 
[corresponding web page](https://cpe.ext.hpe.com/docs/latest/cce/man7/intro_openmp.7.html)

OpenMP **is turned off by default** which is the opposite of how earlier versions the CCE compilers worked.
It is turned on using the `-homp` (Fortran only) or `-fopenmp` flag.
...
:material-help-circle-outline: `man intro_openacc` or 
[corresponding web page](https://cpe.ext.hpe.com/docs/latest/cce/man7/intro_openacc.7.html)
...
:material-help-circle-outline: `man explain` or
[the corresponding web page](https://cpe.ext.hpe.com/docs/latest/cce/man1/explain.1.html)

Use the explain command to display an explanation of any message issued by the
Fortran compiler. This message will be identified with a code looking like
`ftn-<number>`. You can pass this identifier as an argument to the `explain`
command to find out more about the error. The command also works for errors 
generated by the LibSci libraries.
...
:material-help-circle-outline: `man crayftn` or
[the corresponding web page](https://cpe.ext.hpe.com/docs/latest/cce/man1/crayftn.1.html) and
["Cray Compiler Fortran Reference" web page](https://cpe.ext.hpe.com/docs/latest/getting_started/CPE-CCE-Fortran.html)
...
:material-help-circle-outline: `man craycc` - `man crayCC` and the
["HPE Cray Clang C and C++ Quick Reference" web page](https://cpe.ext.hpe.com/docs/latest/getting_started/CPE-CCE-Clang-Reference.html)
